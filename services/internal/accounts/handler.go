...
func (h *Handler) Routes() http.Handler {
	...
	// The merchant context is only needed by the /stores and /platform routes; lifting it for all is harmless.
	return authx.MerchantMiddleware(mux)
}
...
func (h *Handler) createStore(w http.ResponseWriter, r *http.Request) {
	...
	store, err := h.svc.CreateStore(r.Context(), ownerID, body.Slug, body.DisplayName)
	switch {
	case errors.Is(err, ErrInvalidInput):
		httpx.FailDetail(w, http.StatusBadRequest, "invalid input", err.Error())
		return
