...
| Version | Date | Change |
|---------|------|--------|
| v0.1.0 | 2026-06-08 | Initial build: four Go services, SvelteKit storefront, MongoDB + Redis, no-oversell guarantee with load test and race-tested suite |
| v0.2.0 | 2026-06-09 | Pivot to a multi-store builder: `accounts` service (JWT auth + store registry), `tenantId` isolation across catalog/inventory/orders, gateway tenant resolution + JWT-scoped admin routes, per-store storefront + merchant dashboard. No-oversell now per tenant. |
| v0.3.0 | 2026-06-09 | Front restructured into a real SaaS: marketing landing at `/`, onboarding (`/signup` → `/app`), per-store storefront moved to `/store/[slug]`, merchant dashboard at `/app`, `/admin` reserved for the platform super-admin, preferences in `/configuracion`. Cleaned navbar, centralized brand, accent contrast guard + visible focus. |
